	...
	public override void Colliding(Thing other, float percent, float dt)
	{
		...
		else if (other is PlayerCitizen player)
		{
			if(!player.IsDead)
            {
				...
				Game.PlaySfxNearby("xp", Position, pitch: Utils.Map(player.ExperienceCurrent, player.GetExperienceReqForLevel(player.Level - 1), player.ExperienceRequired, 0.9f, 1.3f, EasingType.QuadIn), volume: 1f, maxDist: 4f);
				Remove ();
			}
		}
