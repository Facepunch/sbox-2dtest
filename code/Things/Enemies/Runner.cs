        ...
        else if (other is PlayerCitizen player)
        {
            if(!player.IsDead)
            {
                ...
                if (IsAttacking && _damageTime > (DAMAGE_TIME / TimeScale))
                {
                    ...
                    if (damageDealt > 0f)
                    {
                        Game.PlaySfxNearby("runner.bite", Position, pitch: Utils.Map(player.Health, player.Stats[StatType.MaxHp], 0f, 0.9f, 0.95f, EasingType.QuadIn), volume: 1f, maxDist: 5.5f);
